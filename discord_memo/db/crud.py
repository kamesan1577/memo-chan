...
# MessageのCRUD操作
def create_message(db: Session, is_binary_data: bool, image_link: str, content: str, message_link: str):
    db_message = Message(
...
def update_message(db: Session, message_id: int, content: str = None, image_link: str = None, message_link: str = None):
    ...
    if db_message:
        ...
        db_message.last_updated_at = datetime.utcnow()
        db.commit()
        db.refresh(db_message)
    return db_message
...
# Tag2MessageのCRUD操作
def create_tag2message(db: Session, tag_id: int, message_id: int, channel_id: int, group_id: int = 0):
...
def delete_tag2message(db: Session, tag2message_id: int):
    db_tag2message = db.query(Tag2Message).filter(Tag2Message.id == tag2message_id).first()
    if db_tag2message:
...
def delete_message_group(db: Session, group_id: int):
    db_message_group = db.query(MessageGroup).filter(MessageGroup.group_id == group_id).first()
    if db_message_group:
