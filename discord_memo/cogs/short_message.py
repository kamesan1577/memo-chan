...
from discord_memo.db.crud import get_tags_by_name
...
class ShortMessage(commands.Cog):
    ...
    async def memo(self, interaction: discord.Interaction, message:str, tags:str, title:str=config.MEMO_TITLE):
        db = SessionLocal()
        try:
            message_data_list = self.message_tools.create_message_data(interaction,message)
            await self.message_tools.add_message(interaction, tags, [message_data_list])


            embed = self.memo_embed.create_embed(message, self.message_tools.get_message_link(interaction), title)
            channel_id_list = []
