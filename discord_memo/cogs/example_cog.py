...
class ExampleCog(commands.Cog):
    ...
    async def send_success(self, interaction: discord.Interaction):
        ...
        if custom_contents:
            await custom_contents.send_embed_success(interaction, 'Success', 'This is a Success message.')

    @app_commands.command(name='error')
    async def send_error(self, interaction: discord.Interaction):
        custom_contents = self.bot.get_cog('CustomContents')
        if custom_contents:
            await custom_contents.send_embed_error(interaction, 'Error', 'This is an Error message.')
